...
export const DEFAULT_SHAKE_DETECTION_PARAMS = {
  ...
  // 初めて検知として採用する。この経路は「weakIntensityCeiling未満」
  ...
  mergeAssumedDepthKm: 10,      // 震度からマグニチュード・到達距離を逆算する
                                 // 際に仮定する震源の深さ(km)。実際の深さは
                                 // 分からないため、ごく浅い地震を想定した
                                 // 単純化(shakeTestSimulation.tsのテスト用
                                 // 震源例と同程度の値)
...
function estimateMaxReachKm(peakIntensity, params) {
  ...
  const magnitude = estimateMagnitudeFromPeakIntensity(peakIntensity, params.mergeAssumedDepthKm);
  const reachKm = estimateReachDistanceKm(magnitude, params.mergeAssumedDepthKm, params.mergeFloorIntensity);
  return reachKm * params.mergeReachMarginFactor;
}
...
function checkAttenuationConsistency(a, b, params) {
  ...
  const magnitudeA = estimateMagnitudeFromPeakIntensity(a.peakIntensity, params.mergeAssumedDepthKm);
  const predictedAtB = calcPeakIntensityForMerge(magnitudeA, params.mergeAssumedDepthKm, distKm);
  const diffAtB = Math.abs(predictedAtB - b.peakIntensity);

  const magnitudeB = estimateMagnitudeFromPeakIntensity(b.peakIntensity, params.mergeAssumedDepthKm);
  const predictedAtA = calcPeakIntensityForMerge(magnitudeB, params.mergeAssumedDepthKm, distKm);
  const diffAtA = Math.abs(predictedAtA - a.peakIntensity);

  return Math.min(diffAtB, diffAtA) <= params.mergeAttenuationToleranceIntensity;
}
...
// 誤って統合し損ねないようにするため)。
function canEventsMerge(a, b, params) {
...
function addPointToEvent(event, point, level, now, params) {
  ...
  if (!event.detectionTimes.has(point.id)) {
    ...
    event.detectionTimes.set(point.id, { lat: point.lat, lon: point.lon, detectedAt, level, intensity: point.latestIntensity });
    ...
    return [...this.events.values()]
      ...
      .map(e => {
        ...
        return {
          ...rest,
          pointIds: [...e.pointIds],
          detections: [...detectionTimes].map(([id, d]) => ({ id, lat: d.lat, lon: d.lon, detectedAt: d.detectedAt, level: d.level, intensity: d.intensity })),
        };
      })

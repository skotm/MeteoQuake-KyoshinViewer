...
export class ShakeDetectionEngine {
  ...
  processTick(valuesMap, now = Date.now()) {
    ...
    //     震度上昇中(microChainと同じmicroRise条件)の観測点を、近傍25km以内の
    //     連結成分(Union-Find)としてクラスタ化し、前tickの対応するクラスタと
    //     比較して「広さ」と「継続的な拡大」の両方を満たせば検知として採用する。
    //     microChainの厳密な速度整合性では拾えない、広範囲にじわじわ広がる
    //     タイプの揺れを補う経路。
    {
      const candidateIds = [];
      for (const point of points.values()) {
        ...
        const microRise = point.intensityDiff >= params.microRiseThreshold
          && (point.avgDiff == null || point.avgDiff >= params.microAvgRiseThreshold);
        if (microRise) candidateIds.push(point.id);
      }
      ...
      for (const cluster of nextClusters) {
        if (cluster.radiusKm < params.broadRiseMinRadiusKm) continue;
        if (cluster.growthStreak < params.broadRiseGrowthTicks) continue;

...
function addPointToEvent(event, point, level, now, params) {
  ...
  // 震源推定用に、この観測点が最初にイベントへ加わった時刻だけを記録する
  // (2回目以降の呼び出しでは上書きしない = あくまで「最初に検知した時刻」)。
  if (!event.detectionTimes.has(point.id)) {
    event.detectionTimes.set(point.id, { lat: point.lat, lon: point.lon, detectedAt: now });
  }
    ...
    return [...this.events.values()]
      ...
      .map(e => {
        ...
        return {
          ...rest,
          pointIds: [...e.pointIds],
          detections: [...detectionTimes].map(([id, d]) => ({ id, lat: d.lat, lon: d.lon, detectedAt: d.detectedAt })),
        };
      })

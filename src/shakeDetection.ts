...
// あった。2つのイベントが物理的に同じ震源から来た可能性を、以下の2条件の
// ANDで判定する。
//
// (1) タイミング整合性(片側のみ): 三角不等式により、2つの起点の検知時刻差は
//     「起点間の距離を最も遅い伝播速度(pWaveMinSpeedKmS)で割った値」を
//     超えられない(超えていれば、共通の震源からは物理的に説明できないほど
//     離れている=別の地震)。逆に時刻差が小さすぎることは、共通震源に対し
//     ほぼ等距離なら普通に起こり得るため、除外条件には使わない(下限なし)。
// (2) 震度減衰整合性: 起点間の距離が、どちらか一方のピーク震度から逆算
//     される最大到達距離(estimateMaxReachKm)を超えないこと。弱いイベント
//     (ノイズに近い)同士では最大到達距離が小さくなるため、遠方のイベント
//     とは統合されにくくなる(タイミング整合性だけでは「距離が離れるほど
//     許容される時刻差も広がる」ため歯止めにならなかった問題への対処)。
function canEventsShareOrigin(a, b, params) {
  ...
  const maxAllowedSec = distKm / params.pWaveMinSpeedKmS + params.mergeTimingMarginSec;

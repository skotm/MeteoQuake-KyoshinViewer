...
export class ShakeDetectionEngine {
  ...
  processTick(valuesMap, now = Date.now()) {
    ...
    //    増えておらず、かつ関与する観測点が軒並み反応が収まっていれば、
    //    ノイズ・瞬間的なスパイクだったとみなして即座に取り消す(通常の
    //    寿命=eventDurationBaseMsを待たない)。判定は2段階:
    ...
    for (const event of this.events.values()) {
      ...
      if (event.pointCount > event.pointCountAtWindowStart) continue; // 観測点が増えた→本物とみなす

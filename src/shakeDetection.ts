 ...
 *
 * 【要件E: 広域継続拡大による微小地震検知】
 * microChain(3点以上の伝播速度一貫性連鎖)は、観測点間の厳密な速度整合性を
 * 要求するため、広範囲(東北全域規模)にわたって震度上昇中の観測点が連続的に
 * 分布し、時間とともに面的に拡大していくタイプの揺れを拾えないことがあった。
 * microChainとは並列の別経路として、震度上昇中の観測点の連結クラスタ(近傍
 * 25km以内で連結)を毎tick追跡し、クラスタが「広く」かつ「継続的に拡大して
 * いる(停滞は許容し、縮小した場合のみ実績をリセット)」場合に検知として
 * 採用する(processTickのステップ3b、broadRiseClustersを参照)。
 * 実データでのフィードバックを受け、候補基準をUIの「震度上昇中」表示と
 * ほぼ同等の緩さ(broadRiseCandidateThreshold)に変更し、初回から十分広い
 * クラスタは継続拡大を待たず即検知するパス(broadRiseImmediateRadiusKm)も
 * 追加した。
 */
...
export const DEFAULT_SHAKE_DETECTION_PARAMS = {
  ...
  // 【要件E】広域継続拡大による微小地震検知(microChainとは並列の別経路)。
  ...
  //
  // 【誤検知対策・再々調整】上記のcandidateThresholdだけでは、密集地域の
  // 相関ノイズ(1〜2tickの瞬間的な跳ね返り)にまだ反応してしまうケースが
  // 残っていた。「現在値 - 直前1tickの値」という瞬間diffではなく、「現在値 -
  // 5〜10秒前の平均」という、過去にずらした窓の平均との比較に変更する。
  // 単発の瞬間的なノイズの跳ね返りでは、5〜10秒前の水準と比べた上昇量が
  // 積み上がりにくく候補になりにくい一方、継続的に上昇し続ける本物の弱い
  // 揺れは数tickあれば十分到達できる。既存のhistory/avgDiff(直近5秒平均との
  // 差、他の4ルールと共有)とは独立した専用の履歴バッファ
  // (point.broadRiseHistory)を新設し、他のルールに影響を与えないようにする
  // (詳細はcomputeBroadRiseDelayedAvgDiffを参照)。
  broadRiseHistoryTicks: 10,          // 遅延窓平均に必要な履歴バッファの長さ
                                       // (秒)。「5〜10秒前」の窓を作るのに
                                       // 必要な最小限の長さ
  broadRiseCandidateThreshold: 0.01, // 広域拡大検知専用の、候補に入れるための
                                      // 最小上昇量(現在値 - 5〜10秒前の平均)。
                                      // 比較対象が5〜10秒分の平均になり単発
                                      // ノイズへの耐性が上がったことを踏まえた
                                      // 値。広さ+継続性+タイミング相関の後段
...
// 【誤検知対策・再々調整】広域拡大検知の候補判定を「現在値 - 直前1tickの
// 値」ではなく「現在値 - 5〜10秒前の平均」に変更するための計算。
// point.broadRiseHistory(直近broadRiseHistoryTicks件、古い順)のうち、
// 古い方の半分(6〜10tick前に相当)を「5〜10秒前の窓」とみなして平均する。
// バッファがbroadRiseHistoryTicksに満たない(観測開始直後・復帰直後等)場合は
// 判定材料不足としてnullを返す。呼び出し側はこれを「候補にしない」(厳しい
// 側)として扱う(他の判定不能時の「本物とみなす」側とは方針が異なる点に注意)。
function computeBroadRiseDelayedAvgDiff(point, params) {
  if (point.latestIntensity == null) return null;
  const buf = point.broadRiseHistory;
  if (buf.length < params.broadRiseHistoryTicks) return null; // データ不足→候補にしない

  const nearWindowTicks = params.broadRiseHistoryTicks - params.historyLength; // 直近側(5秒)の除外幅
  const delayedWindow = buf.slice(0, buf.length - nearWindowTicks); // 6〜10tick前(古い方半分)
  if (delayedWindow.length === 0) return null;

  let sum = 0;
  for (const v of delayedWindow) sum += v;
  const delayedAvg = sum / delayedWindow.length;
  return point.latestIntensity - delayedAvg;
}
  ...
  initialize(stations) {
    ...
    for (const s of targetStations) {
      ...
      this.points.set(s.id, {
        ...
        // 【誤検知対策・再々調整】広域拡大検知専用の履歴バッファ(直近
        // broadRiseHistoryTicks件の値)。既存のhistory(5秒平均、他の4ルールと
        // 共有)とは独立させ、「5〜10秒前の平均」という、直近を含まない
        // 過去にずらした窓の平均を計算するために使う
        // (computeBroadRiseDelayedAvgDiff参照)。
        broadRiseHistory: prev ? prev.broadRiseHistory : [],
        nearPoints: [],
      });
    }
    for (const s of targetStations) {
  ...
  processTick(valuesMap, now = Date.now()) {
    ...
    for (const point of points.values()) {
      ...
      if (point.latestIntensity != null) {
        ...
        // 【誤検知対策・再々調整】広域拡大検知専用の履歴バッファも同様に
        // 積んでおく(既存historyとは独立、長さもbroadRiseHistoryTicksで別管理)。
        point.broadRiseHistory.push(point.latestIntensity);
        if (point.broadRiseHistory.length > params.broadRiseHistoryTicks) point.broadRiseHistory.shift();
      }
      ...
      // 【要件E追加対策・再々調整】広域拡大検知の候補基準を満たしている間の
      // 開始時刻を追跡する。reactionStartAtとは基準が異なるため別フィールドで
      // 管理する。判定は「現在値 - 5〜10秒前の平均」(computeBroadRiseDelayed
      // AvgDiff)を使う。バッファ不足でnullが返る場合は候補にしない(厳しい側)。
      const broadRiseDelayedAvgDiff = computeBroadRiseDelayedAvgDiff(point, params);
      const broadRiseCandidate = broadRiseDelayedAvgDiff != null && broadRiseDelayedAvgDiff >= params.broadRiseCandidateThreshold;
    ...
    //    連鎖・ステップ3bの広域継続拡大検知(要件E)で扱うためここでは対象外に
    //    する。孤立点(近傍なし)はそもそも近傍N点中K点方式が組めないため、
    //    従来どおりisolatedRiseThresholdで単独判定する。それ以外(震度0以上、
    //    または震度0未満でも孤立点)は、detectionRulesのアンサンブルで判定
    //    する(近傍の裏付けもルールに組み込まれているため、任意の1点との
    //    時間窓一致は見ない)。
    for (const point of points.values()) {

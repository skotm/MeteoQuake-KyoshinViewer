...
export const DEFAULT_SHAKE_DETECTION_PARAMS = {
  ...
  broadRiseCandidateThreshold: 0.02, // 広域拡大検知専用の、候補に入れるための
                                      // 最小上昇量(前tickとの差)。
                                      // microRiseThreshold(0.08)より大幅に
                                      // 緩め、UIの「震度上昇中」表示とほぼ
                                      // 同じ感度にする。avgDiffによる足切りは
                                      // 行わない(広さ+継続性で別途ノイズを
                                      // 弾くため)。
...
// 【要件E追加対策】密集地域での相関ノイズ(付近の観測点がたまたま同時に
// 震度上昇する)対策。クラスタ内で最も早く広域拡大検知の候補になった点を
// 「起点」とみなし、各メンバーの「起点からの距離」と「起点より遅れて候補に
// なった時間」の相関係数(ピアソン)を計算する。本物の地震であれば、伝播に
// 有限の速度がかかるため距離が離れるほど遅れて上昇するはず(正の相関)。
// 一方、相関ノイズ(交通振動・降雨等)は地理的に相関しつつもほぼ同時多発的に
// 現れるため、距離によらず遅れがバラつき、相関が弱くなりやすい。
// メンバー数が少ない・値が退化している(全点が同時刻など)場合はnullを返し、
// 判定材料が無いことを示す(呼び出し側は「本物とみなす」側に倒す)。
function computeBroadRiseTimingCorrelation(memberIds, points, params) {
  const withSince = memberIds
    .map((id) => points.get(id))
    .filter((p) => p && p.broadRiseCandidateSince != null);
  if (withSince.length < params.broadRiseTimingMinSamples) return null;

  let origin = withSince[0];
  for (const p of withSince) {
    if (p.broadRiseCandidateSince < origin.broadRiseCandidateSince) origin = p;
  }

  const xs = []; // 起点からの距離(km)
  const ys = []; // 起点より遅れて候補になった時間(秒)
  for (const p of withSince) {
    xs.push(haversineKm(origin.lat, origin.lon, p.lat, p.lon));
    ys.push((p.broadRiseCandidateSince - origin.broadRiseCandidateSince) / 1000);
  }
  ...
  if (varX === 0 || varY === 0) return null; // 全点が同距離/同時刻など、退化したケース
...
export class ShakeDetectionEngine {
  ...
  processTick(valuesMap, now = Date.now()) {
    ...
    //     2段構えで別途ノイズを弾く設計のため問題ない。
    {
      ...
      for (const point of points.values()) {
        if (point.latestIntensity == null || point.latestIntensity >= params.microChainIntensityCeiling) continue;
        if (point.intensityDiff >= params.broadRiseCandidateThreshold) candidateIds.push(point.id);
      }

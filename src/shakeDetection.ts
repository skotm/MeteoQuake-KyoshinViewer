...
export class ShakeDetectionEngine {
  ...
  processTick(valuesMap, now = Date.now()) {
    ...
    //    【対策: 離島・海域を挟んだイベント統合】通常の距離ベース判定
    //    (rectMinDistanceKm <= mergeDistanceKm)に加えて、物理的な整合性
    //    (canEventsShareOrigin: タイミング+震度減衰)が認められる場合も
    //    OR条件で統合する。観測点の空白地帯を挟むケースをカバーするため。
    let merged = true;
    while (merged) {
      merged = false;
      ...
      for (let i = 0; i < eventList.length; i++) {
        ...
        for (let j = i + 1; j < eventList.length; j++) {
          ...
          if (rectMinDistanceKm(a, b) <= params.mergeDistanceKm || canEventsShareOrigin(a, b, params)) {
            mergeEventInto(a, b);

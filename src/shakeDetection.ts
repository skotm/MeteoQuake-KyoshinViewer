 ...
 * 検知の速さへの影響は最小限に抑えている。
 *
 * さらに、震度0未満のごく微小な変化については、上記の「近傍が同時期に
 * 急上昇」判定の代わりに、P波の伝播速度に基づく連鎖検知を使う。観測点間の
 * 距離と検出時刻の差から伝播速度を逆算し、それが物理的に妥当な範囲
 * (shakeTestSimulation.tsのP波速度6.8km/s前後)であれば地震波の可能性が
 * 高いとみなす。ただし2点だけの速度一致は偶然のこともあるため、3点以上
 * (A→B→C)の連鎖で速度の一貫性が取れて初めて検知として採用する。
 *
...
export const DEFAULT_SHAKE_DETECTION_PARAMS = {
  ...
  // 初めて検知として採用する。この経路は「震度0未満」の範囲でのみ、
  // corroborationWindowTicksによる同時多発判定の代わりに使う
  // (震度0以上の通常の急上昇検知には影響しない)。
  tickIntervalSeconds: 1,      // 1tickが何秒に相当するか(実データの更新間隔)
  microRiseThreshold: 0.12,    // 震度0未満の観測点で、これ以上の上昇があれば
...
export class ShakeDetectionEngine {
  ...
  processTick(valuesMap, now = Date.now()) {
    ...
    for (const point of points.values()) {
      if (point.latestIntensity == null || point.latestIntensity >= 0) continue;
    ...
    // 4) 同時多発判定 → イベント割当。震度0未満・かつ近傍がある点は、
    //    ステップ3の伝播速度連鎖で扱うためここでは対象外にする
    //    (corroborationWindowTicksでの同時多発判定を伝播速度チェックに
    //    置き換える対象は「近傍を使う判定」のみ。孤立点はそもそも連鎖を
    //    組めないため、震度0未満でも従来どおりisolatedRiseThresholdで扱う)。
    //    履歴平均・スコア積み上げを挟まず、このtickで急上昇した点について
    //    その場で判定する。
    for (const point of points.values()) {
      if (point.lastRiseTick !== tick) continue;
      const isolatedForCheck = point.nearPoints.length === 0;
      if (point.latestIntensity < 0 && !isolatedForCheck) continue;
      ...
      const triggered = isolated
        ? (point.intensityDiff >= params.isolatedRiseThreshold && isolatedAvgOk)

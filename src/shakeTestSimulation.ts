  ...
  for (const [id, st] of perStation) {
    ...
    if (elapsedMs < st.sArrivalMs) {
      ...
      if (sinceS < RISE_DURATION_MS) {
        ...
        // 保持後、平常値まで指数的に減衰
        const t = (sinceS - RISE_DURATION_MS - HOLD_DURATION_MS) / DECAY_DURATION_MS;
...
export function isShakeTestFinished(perStation, elapsedMs) {
  ...
  return elapsedMs > maxSArrivalMs + RISE_DURATION_MS + HOLD_DURATION_MS + DECAY_DURATION_MS + 3000;
}

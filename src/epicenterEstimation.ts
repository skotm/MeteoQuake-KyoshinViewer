 ...
 *   - 誤差レベル: 到達時刻の分散(本編)に加えて、震度と震央距離の対数の
 *     相関係数から「震度の減衰パターンが候補震源から見て物理的にもっともらしいか
 *     (遠いほど震度が低いか)」のペナルティを加算する。
 * 気象庁の改良IPF法のような、振幅から距離を定量的に逆算する校正済みモデルでは
 * なく、あくまで観測点間の相対的な手がかりとして使う簡易版であることに注意。
 *
 ...
 * - 震度の減衰パターンによるペナルティ(computeAmplitudeConsistencyPenalty)も
 *   検知済み観測点数に対してO(n)(ピアソン相関係数の計算)で済み、候補ごとの
 *   評価コストを大きく増やさないようにしている。
 */

import { P_WAVE_SPEED_KM_S } from "./shakeTestSimulation";
...
// 過ぎてしまう。着未着法のペナルティ(整数の加算、多くても数百)と同程度の
// オーダーを目安に、暫定的にこの値にしている(要調整)。
const AMPLITUDE_PENALTY_WEIGHT = 150;
// 相関係数を計算する最低点数(2点だと機械的にr=±1になり意味を持たないため)。
...
// 揺れの広がり方(震度の減衰パターン)が、候補震源から見て物理的にもっとも
// らしいか(遠い観測点ほど震度が低いか)を評価する。震央距離の対数
// (log10(震源距離))と震度のピアソン相関係数rを使い、理想的な負の相関
// (r=-1、遠いほど震度が低い)からのズレをペナルティにする:
// r=-1で0、r=0(震度と距離が無関係)でAMPLITUDE_PENALTY_WEIGHT、
// r=+1(震度の高低が距離と逆転=あり得ない)で4倍、の連続関数。
// 観測点数が少ない、または距離・震度にほぼバラつきが無い場合は相関が
// 不安定なため計算をスキップする(0を返す=このペナルティは効かせない)。
function computeAmplitudeConsistencyPenalty(candidate, detections) {
  ...
  for (const d of detections) {
    ...
    const distHypo = Math.sqrt(distKm * distKm + candidate.depthKm * candidate.depthKm);
    xs.push(Math.log10(Math.max(distHypo, 0.1)));
    ys.push(d.level);
  }
  if (xs.length < MIN_POINTS_FOR_AMPLITUDE_CHECK) return 0;
  ...
  if (varX < 1e-9 || varY < 1e-9) return 0; // バラつきが無く相関が不安定な場合はスキップ

  const r = cov / Math.sqrt(varX * varY);
  return AMPLITUDE_PENALTY_WEIGHT * (r + 1) * (r + 1);
}
...
function computeErrorLevel(candidate, detections, firstDistKm, allStations, now) {
  ...
  // 分散だけでは区別しにくい候補同士を、震度が距離ともっともらしく対応して
  // いるかで追加的に絞り込む(詳細はcomputeAmplitudeConsistencyPenalty参照)。
  errorLevel += computeAmplitudeConsistencyPenalty(candidate, detections);
